...
const ItemCard = React.memo(function ItemCard({ file, index, onRemove, onPreview, isOverlay }: { file: File, index?: number, onRemove?: () => void, onPreview?: () => void, isOverlay?: boolean }) {
    ...
    const cacheKey = `${file.name}-${file.size}-${file.lastModified}`;
    const [previewUrl, setPreviewUrl] = useState<string | null>(() => thumbnailCache.get(cacheKey) || null);
    ...
    return (
        <div
            ...
            className={`group relative aspect-[3/4] bg-white dark:bg-slate-900 border ${isOverlay ? 'border-primary shadow-2xl scale-105 cursor-grabbing' : 'border-slate-200 dark:border-slate-800 hover:border-primary/50 cursor-grab'} rounded-2xl overflow-hidden transition-colors shadow-sm hover:shadow-lg`}
        >
            ...
            {/* Hover Overlay Actions - Don't render if it is the drag overlay */}
            {!isOverlay && (
                <div className="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 transition-opacity duration-200 flex flex-col justify-between p-3">
                    ...
                    <div className="bg-white/95 dark:bg-slate-800/95 rounded-xl p-3 shadow-lg">
